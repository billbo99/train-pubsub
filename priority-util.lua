...
function surface_copy(event)
    ...
    safe_priority_import(storage.newpriority[from_surface], to_surface)
    -- for icon1,surf in pairs(storage.newpriority[from_surface]) do
    --     for icon2,surface in pairs(surf) do
    ...
    --     remote.call("TSM-API","define_new_priority",icon1,icon2,{},to_surface)
    --  --
    --   --  helpers.write_file("wc",serpent.block(surface.wc),{comment=false})
    --   if surface.wc ~= nil then
    --     remote.call("TSM-API","update_wc",icon1,icon2,surface.wc,to_surface)
    --   else
    --     game.print(icon1 .. icon2 .. " wc is nil")
...
function surface_delete(event)
    ...
    --  helpers.write_file("pre-delete-priority",serpent.block(storage.newpriority),{comment=false})
    storage.newpriority[delete_surface] = {}
end
...
function surface_export(event)
    ...
    helpers.write_file(export_file, game.encode_string(game.table_to_json(storage.newpriority[export_surface])), false,
        event.player_index)
...
function surface_import(event)
    ...
    if game.decode_string(import_file) then
        ...
        if type(tmp_table) == "table" then
            ...
            storage.newpriority[import_surface] = {}
            safe_priority_import(tmp_table, import_surface)
            --   storage.newpriority[import_surface] = tmp_table
