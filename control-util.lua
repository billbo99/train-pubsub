function Check_train_config(train, station)
	...
	for _, config in pairs(station.surface.find_entities_filtered{area={{x=station.position.x - 2,y=station.position.y - 2},{x=station.position.x + 2,y=station.position.y + 2}},type = "constant-combinator", name = "train-config"}) do
...
function build_schedule(train, backer_name, priority)
	...
	if priority.wc then
		...
		if priority.wc.inactivity == true then
			...
			record.wait_conditions[#record.wait_conditions+1] = {type = "inactivity", compare_type = compare, ticks = ticks}
		end
		...
		--	game.print(priority.resource.type)
			...
			record.wait_conditions[#record.wait_conditions+1] = {type = count_typ, compare_type = compare, condition = {comparator = comp_sym[priority.wc.count_ddn], first_signal = {type = priority.resource.type, name = priority.resource.name}, constant = priority.wc.count_amt}}
		end
	else
			...
			if global.newrequests[surface] ~= nil then
				if global.newrequests[surface][backer_name] ~= nil then
					if global.newrequests[surface][backer_name][key] ~= nil then
						global.newrequests[surface][backer_name][key].hide = global.newpublishers[surface][backer_name][key].hide
					end
				end
			end
		end
...
function update_priority_wc(event)
	...
	if string.sub(element.name,1,4) == "new_" then
		...
		wc = global.newpriority[player.surface.name][global.player[player.index].resource][global.player[player.index].id].wc
	end
	if name == "rb_or" then
...
function save_priority_wc(event)
	...
	elseif name == "efinc" then
		if wc.inc_ef == true then global.player[player.index].wc.inc_ef = false
		elseif wc.inc_ef == false then global.player[player.index].wc.inc_ef = true end
	elseif name == "empty" then
		if wc.empty == false then
	...
	elseif name == "inactivity" then
		if wc.inactivity == true then global.player[player.index].wc.inactivity = false
		elseif wc.inactivity == false then global.player[player.index].wc.inactivity = true end
	elseif name == "wait_timer" then
		if wc.wait_timer == true then global.player[player.index].wc.wait_timer = false
		elseif wc.wait_timer == false then global.player[player.index].wc.wait_timer = true end
	elseif name == "count" then
		if wc.count == true then global.player[player.index].wc.count = false
		elseif wc.count == false then global.player[player.index].wc.count = true end
	end
end
...
script.on_event(defines.events.on_gui_value_changed, function(event)
	...
	if event.element.name == "inact_slider" then
		global.newpriority[player.surface.name][global.player[player.index].resource][global.player[player.index].id].wc.inact_int = value
	...
	elseif event.element.name == "wait_slider" then
		global.newpriority[player.surface.name][global.player[player.index].resource][global.player[player.index].id].wc.wait_int = value
...
script.on_event(defines.events.on_gui_text_changed, function(event)
	...
	elseif event.element.name == "inact_int" or
		...
		elseif event.element.name == "process_priority" then
			...
			global.newpublishers[player.surface.name][global.cur_publisher[event.player_index].backer_name][ global.cur_publisher[event.player_index].key ].proc_priority = value
						...
						--	if request.proc_priority ~= "1" then
							...
							--	debugp("found reqpri proc priority")
								if reqpri.proc_priority > request.proc_priority then
									reqpri = {proc_priority = request.proc_priority, request = request, priority = priority, i = i, tick = request.tick}
								elseif (reqpri.tick > request.tick and reqpri.proc_priority == request.proc_priority) then
										reqpri = {proc_priority = request.proc_priority, request = request, priority = priority, i = i, tick = request.tick}
								end
							else
								reqpri = {proc_priority = request.proc_priority, request = request, priority = priority, i = i, tick = request.tick}
	...
	if event.source.name == "train-publisher" and event.destination.name == "train-publisher" then
		...
		local status,err = pcall(function()
			if source ~= {} and destination ~= {} then
				global.newpublishers[player.surface.name][destination.backer_name][destination.i] = table.deepcopy(global.newpublishers[player.surface.name][source.backer_name][source.i])
				global.newpublishers[player.surface.name][destination.backer_name][destination.i].entity = event.destination
				...
				if global.newpublishers[player.surface.name][source.backer_name][source.i].proc_priority ~= nil then
					global.newpublishers[player.surface.name][destination.backer_name][destination.i].proc_priority = global.newpublishers[player.surface.name][source.backer_name][source.i].proc_priority
				else
